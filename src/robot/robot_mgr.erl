...
init([]) ->
    ?INFO("[~p] 启动完成", [?MODULE]),
    ets:new(?ETS_ROBOT, [set, public, named_table, {keypos, #user_robot.username}, {write_concurrency, auto}]),
    {ok, #state{}}.

handle_call({start, Username}, _From, State = #state{}) ->
    {reply, start_robot(Username), State};
        ...
        [] ->
            case ets:match_object(?ETS_ROBOT, #user_robot{username = Username, _ = '_'}) of
                [] ->
                    ...
                    ets:insert(?ETS_ROBOT, #user_robot{username = Username, pid = Pid}),
                    {ok, Pid};
                _ ->
                    {error, already}
            end;
